...
# Copied from transformers.models.led.modeling_led.LEDLearnedPositionalEmbedding with LED -> LayoutLED
class LayoutLEDLearnedPositionalEmbedding(nn.Embedding):
    """
...
# Copied from transformers.models.led.modeling_led.LEDEncoderSelfAttention with LED->LayoutLED
class LayoutLEDEncoderSelfAttention(nn.Module):
...
# Copied from transformers.models.led.modeling_led.LEDEncoderAttention with LED->LayoutLED
class LayoutLEDEncoderAttention(nn.Module):
    def __init__(self, config, layer_id):
        super().__init__()
        self.longformer_self_attn = LayoutLEDEncoderSelfAttention(config, layer_id=layer_id)
...
# Copied from transformers.models.led.modeling_led.LEDDecoderAttention with LED->LayoutLED
class LayoutLEDDecoderAttention(nn.Module):
...
# Copied from transformers.models.led.modeling_led.LEDEncoderLayer with LED->LayoutLED
class LayoutLEDEncoderLayer(nn.Module):
    def __init__(self, config: LayoutLEDConfig, layer_id: int):
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = LayoutLEDEncoderAttention(config, layer_id)
...
# Copied from transformers.models.led.modeling_led.LEDDecoderLayer with LED->LayoutLED
class LayoutLEDDecoderLayer(nn.Module):
    def __init__(self, config: LayoutLEDConfig):
        super().__init__()
        self.embed_dim = config.d_model

        self.self_attn = LayoutLEDDecoderAttention(
            embed_dim=self.embed_dim,
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = LayoutLEDDecoderAttention(
            self.embed_dim,
...
class LayoutLEDPreTrainedModel(PreTrainedModel):
    config_class = LayoutLEDConfig
    base_model_prefix = "layout_led"
    ...
    def dummy_inputs(self):
        ...
        bbox = torch.tensor(
            [[[0, 0, 0, 0]] * len(input_ids[0])] * len(input_ids), device=self.device
        )
        dummy_inputs = {
...
@dataclass
# Copied from transformers.models.led.modeling_led.LEDEncoderBaseModelOutput with LED->LayoutLED
...
@dataclass
class LayoutLEDSeq2SeqModelOutput(ModelOutput):
    """
...
@dataclass
class LayoutLEDSeq2SeqLMOutput(ModelOutput):
    """
...
class LayoutLEDEncoder(LayoutLEDPreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer is a
    [`LayoutLEDEncoderLayer`].

    Args:
        config: LayoutLEDConfig
        embed_tokens (nn.Embedding): output embedding
    """

    def __init__(self, config: LayoutLEDConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.embed_positions = LayoutLEDLearnedPositionalEmbedding(
            self.max_source_positions,
            embed_dim,
        )
        self.layers = nn.ModuleList([LayoutLEDEncoderLayer(config, i) for i in range(config.encoder_layers)])
...
# Copied from transformers.models.led.modeling_led.LEDDEcoder with LED->LayoutLED
class LayoutLEDDecoder(LayoutLEDPreTrainedModel):
    """
    ...
    def __init__(self, config: LayoutLEDConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([LayoutLEDDecoderLayer(config) for _ in range(config.decoder_layers)])
...
class LEDModel(LayoutLEDPreTrainedModel):
    def __init__(self, config: LayoutLEDConfig):
        super().__init__(config)
        ...
        self.encoder = LayoutLEDEncoder(config, self.shared)
        self.decoder = LayoutLEDDecoder(config, self.shared)
    ...
    def forward(
        ...
        return LayoutLEDSeq2SeqModelOutput(
...
class LayoutLEDForConditionalGeneration(LayoutLEDPreTrainedModel):
    ...
    def forward(
        ...
        return LayoutLEDSeq2SeqLMOutput(
            loss=masked_lm_loss,
            logits=lm_logits,
