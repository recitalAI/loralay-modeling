...
from transformers.models.led.modeling_led import LEDModel, LEDForConditionalGeneration

class LEDHeterformer(LEDModel):
    def __init__(self, config):
        super(LEDHeterformer, self).__init__(config)
        if config.attention_mode == 'n2':
            pass  # do nothing, use BertSelfAttention instead
        else:
            for i, layer in enumerate(self.encoder.layers):
                layer.self_attn.longformer_self_attn = LEDHeterformerSelfAttention(config, layer_id=i)


class LEDHeterformerForConditionalGeneration(LEDForConditionalGeneration):
    def __init__(self, config):
        super(LEDHeterformerForConditionalGeneration, self).__init__(config)
        if config.attention_mode == 'n2':
            pass  # do nothing, use BertSelfAttention instead
        else:
...
class LEDHeterformerSelfAttention(nn.Module):
    def __init__(self, config, layer_id):
        super(LEDHeterformerSelfAttention, self).__init__()
